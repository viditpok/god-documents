    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        maxBeliefs = [(ghostDist.argMax(), ghostDist[ghostDist.argMax()]) for ghostDist in livingGhostPositionDistributions]

        bestAction = (None, float("inf"))
        for action in legal:
            for ghost in maxBeliefs:
                successorPosition = Actions.getSuccessor(pacmanPosition, action)
                actionDistance = self.distancer.getDistance(successorPosition, ghost[0])
                if actionDistance < bestAction[1]:
                    bestAction = (action, actionDistance)
