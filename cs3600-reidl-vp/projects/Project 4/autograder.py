...
def q2Test(testFile):
    if 'sigmoidDeriv' in testFile.name:
        ...
        if 'update' in testFile.name:
            ...
            currWeights = sPercep.weights[:]
            sPercep.updateWeights(examples[0][0], 0, 0)
            if not sameList(currWeights, sPercep.weights, 0):
                raise Exception("If there is no error or alpha, then updateWeights should not change the weights!")
