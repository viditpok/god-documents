...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        for n in range(len(self.corners)):
            self.goal_dict[self.corners[n]] = n
    ...
    def isGoalState(self, state):
        ...
        goal_list = list(state[1])
        for corner in self.corners:
            if corner == state[0]:
                goal_list[self.goal_dict[corner]] = 1
        
        if all(value == 1 for value in goal_list):
            return True
        else:
            return False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                # convert tuple to list to modify if current location is a corner
                goal_tuple = state[1]
                goal_list = list(goal_tuple)
                for corner in self.corners:
                    if corner == state[0]:
                        goal_list[self.goal_dict[corner]] = 1
                ...
                nextState = ((nextx, nexty), goal_tuple)
                cost = self.costFn
                successors.append((nextState, action, cost))
...
def foodHeuristic(state, problem):
    ...
    seperation = 0
    pointX = foodGrid.asList()[0]
    pointY = foodGrid.asList()[0]
    ...
    XDistance = util.manhattanDistance(position, pointX)
    YDistance = util.manhattanDistance(position, pointY)

    return min(XDistance, YDistance) + seperation
