...
class GUIWindow:
    ...
    def drawOccupied(self):
        for block in self.occupied:
            if (self.robot and block in self.robot.explored_cells) or (
                self.program_state == "tasks"
            ):
    ...
    def drawMarkers(self):
        ...
        for marker in self.markers:
            if (
                marker[0:2] not in self.robot.explored_cells
                and self.program_state == "exploration"
            ):
                continue
            marker_x, marker_y, marker_h = self.grid.parse_marker_info(
                marker[0], marker[1], marker[2]
            )

            if (
                self.program_state == "tasks"
                and marker not in self.robot.markers_found_or_picked
            ):
            ...
            if marker[2] == "U":
                ...
                self.colorLine(
                    (marker_x, marker_y),
                    (marker_x + arrow_head_x, marker_y + arrow_head_y),
                    linewidth=2,
                    color="#222222",
                )
            elif marker[2] == "D":
            ...
            self.colorRectangle(
                (marker_x + c1x, marker_y + c1y),
                (marker_x + c2x, marker_y + c2y),
                "#008c8c",
            )
    ...
    def _show_robot(self, robot):
        ...
        self.colorLine(
            (robot.x, robot.y),
            (robot.next_coord[0], robot.next_coord[1]),
            color="#222222",
            linewidth=2,
            dashed=True,
        )
    ...
    def colorCell(self, location, color):
        ...
        self.canvas.create_rectangle(
            coords[0],
            coords[1],
            coords[0] + self.grid.scale,
            coords[1] + self.grid.scale,
            width=1,
            fill=color,
        )
    ...
    def colorRectangle(self, corner1, corner2, color):
        ...
        self.canvas.create_rectangle(
            coords1[0], coords1[1], coords2[0], coords2[1], width=0, fill=color
        )

    def colorCircle(self, location, color, dot_size=5):
        x0, y0 = (
            location[0] * self.grid.scale - dot_size,
            (location[1]) * self.grid.scale - dot_size,
        )
        x1, y1 = (
            location[0] * self.grid.scale + dot_size,
            (location[1]) * self.grid.scale + dot_size,
    ...
    def colorLine(self, coord1, coord2, color="black", linewidth=1, dashed=False):
        if dashed:
            self.canvas.create_line(
                coord1[0] * self.grid.scale,
                (coord1[1]) * self.grid.scale,
                coord2[0] * self.grid.scale,
                (coord2[1]) * self.grid.scale,
                fill=color,
                width=linewidth,
                dash=(5, 3),
            )
    ...
    def colorTriangle(self, location, heading_deg, color, tri_size):
        ...
        hrot = (
            hx + location[0] * self.grid.scale,
            -hy + (location[1]) * self.grid.scale,
        )
        lrot = (
            lx + location[0] * self.grid.scale,
            -ly + (location[1]) * self.grid.scale,
        )
        rrot = (
            rx + location[0] * self.grid.scale,
            -ry + (location[1]) * self.grid.scale,
        )
    ...
    def start(self):
        ...
        self.canvas = Canvas(
            master,
            width=self.grid.width * self.grid.scale,
            height=self.grid.height * self.grid.scale,
            bd=0,
            bg=bg_color,
        )
