...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    pad_width = 1
    image_padded = np.pad(image_bw, pad_width, mode="constant", constant_values=0)
    ...
    for y in range(1, padded_height - 1):
        for x in range(1, padded_width - 1):
            patch = image_padded[y - 1 : y + 2, x - 1 : x + 2]

            Ix[y - 1, x - 1] = np.sum(patch * SOBEL_X_KERNEL)
            Iy[y - 1, x - 1] = np.sum(patch * SOBEL_Y_KERNEL)
...
def nms_maxpool_pytorch(
    ...
    median_value = torch.median(R_torch)
    R_thresholded = torch.where(R_torch >= median_value, R_torch, torch.tensor(0.0))

    max_pool = nn.MaxPool2d(
        ksize, stride=1, padding=(ksize - 1) // 2, return_indices=True
    )
    pooled, indices = max_pool(R_thresholded.unsqueeze(0).unsqueeze(0))

    local_maxima = R_thresholded == pooled.squeeze()

    R_local_maxima = R_torch * local_maxima.float()

    confidences, idxs = torch.topk(R_local_maxima.flatten(), k)

    y, x = torch.div(
        idxs, R_local_maxima.size(1), rounding_mode="floor"
    ), idxs % R_local_maxima.size(1)
...
def get_harris_interest_points(
    ...
    harris_response_map = compute_harris_response_map(image_bw)

    harris_response_map = (harris_response_map - harris_response_map.min()) / (
        harris_response_map.max() - harris_response_map.min()
    )

    x, y, c = nms_maxpool_pytorch(harris_response_map, k, ksize=7)
