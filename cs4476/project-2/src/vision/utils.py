...
def draw_intensity_matches(img1,img2,matches,window_size=128):
    """
    Converts patch points from top-left corner to patch midpoints and obtains correspondence lines.

    Args:
        img1: A numpy array for the first image
        img2: A numpy array for the second image
        matches: A numpy array containing the top-left corner points of intensity match windows in the form [[img1_x, img1_y],[img2_x, img2_y]].
    Returns:
        newImg: A numpy array for the final image with img1 on the left, img2 on the right and intensity-based correspondence lines.
    """
    min_height = min(img1.shape[0], img2.shape[0])
    min_width = min(img1.shape[1], img2.shape[1])
    img1_res = img1[:min_height, :min_width,:]
    img2_res = img2[:min_height, :min_width,:]
    X1, Y1, X2, Y2 = [], [], [], []
    
    for match in matches:
        X1.append(match[0][0]+window_size//2)
        Y1.append(match[0][1]+window_size//2)
        X2.append(match[1][0]+ window_size//2)
        Y2.append(match[1][1]+window_size//2)
    ...
    return show_correspondence_lines(img1_res, img2_res, X1, Y1, X2, Y2, line_colors=None)
