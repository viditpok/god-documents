...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        kernel_np = create_Gaussian_kernel_2D(cutoff_frequency)
        kernel_np = np.tile(kernel_np[np.newaxis, np.newaxis, :, :], (self.n_channels, 1, 1, 1))
        kernel = torch.Tensor(kernel_np)
        return kernel
