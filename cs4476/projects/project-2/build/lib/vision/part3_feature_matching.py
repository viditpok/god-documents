...
def compute_feature_distances(
    ...
    dists = np.sqrt(
        np.sum((features1[:, np.newaxis, :] - features2[np.newaxis, :, :]) ** 2, axis=2)
    )
    return dists
...
def match_features_ratio_test(
    ...
    ratios = (
        dists[np.arange(dists.shape[0]), idx_sorted[:, 0]]
        / dists[np.arange(dists.shape[0]), idx_sorted[:, 1]]
    )

    good_ratios = ratios < 0.8
    matches = np.column_stack(
        (np.arange(dists.shape[0])[good_ratios], idx_sorted[good_ratios, 0])
    )

    confidences = 1 - ratios[good_ratios]
