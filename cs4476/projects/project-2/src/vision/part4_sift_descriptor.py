    ...
    for i in range(4):
        for j in range(4):
            ...
            cell_hist, _ = np.histogram(
                cell_orientations, bins=bin_edges - 1e-5, weights=cell_magnitudes
            )
...
def get_SIFT_descriptors(
    ...
    for i in range(k):
        descriptor = get_feat_vec(
            X[i], Y[i], magnitudes, orientations, feature_width
        ).flatten()
        fvs[i, :] = descriptor
...
def rotate_image(image: np.ndarray, angle: int) -> np.ndarray:
    ...
    orig_center_x, orig_center_y = (width - 1) / 2.0, (height - 1) / 2.0

    new_center_x, new_center_y = (new_width - 1) / 2.0, (new_height - 1) / 2.0

    for y_out in range(new_height):
        for x_out in range(new_width):

            x_c = x_out - new_center_x
            y_c = y_out - new_center_y

            x_orig = (cos_angle * x_c - sin_angle * y_c) + orig_center_x
            y_orig = (sin_angle * x_c + cos_angle * y_c) + orig_center_y

            x_orig, y_orig = int(round(x_orig)), int(round(y_orig))

            if 0 <= x_orig < width and 0 <= y_orig < height:
                rotated_image[y_out, x_out] = image[y_orig, x_orig]
            else:

                rotated_image[y_out, x_out] = 0
