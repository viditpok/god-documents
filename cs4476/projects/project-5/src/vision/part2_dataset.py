...
def make_dataset(
    split: str, data_root: str, data_list_fpath: str
) -> List[Tuple[str, str]]:
    """
    ...
    with open(data_list_fpath, "r") as file:
        for line in file:
            ...
            image_label_list.append(
                (os.path.join(data_root, image), os.path.join(data_root, label))
            )
...
class SemData(Dataset):
    def __init__(
        self, split: str, data_root: str, data_list_fpath: str, transform=None
    ) -> None:
        """
...
class KittiData(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        image = cv2.imread(os.path.join(self.train_path, image_path), cv2.IMREAD_COLOR)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
